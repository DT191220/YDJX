...
import { generateVoucherNo, getActiveHeadquarterConfig, calculateHeadquarterAmount, getSubjectCodes } from './finance';
...
router.get('/student/:studentId', async (req: Request, res: Response) => {
  try {
    ...
    const [rows] = await pool.query(
      'SELECT * FROM payment_records WHERE student_id = ? ORDER BY payment_date DESC, created_at DESC',
      [studentId]
    );
...
router.post('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [result] = await connection.query<ResultSetHeader>(
      'INSERT INTO payment_records (student_id, amount, payment_date, payment_method, operator, notes) VALUES (?, ?, ?, ?, ?, ?)',
      [student_id, amountNum, payment_date, payment_method, operator, notes]
    );
    ...
    try {
      // 获取科目映射
      const subjectMapping = await getSubjectCodes(['BANK_DEPOSIT', 'TUITION_INCOME'], connection);
      ...
      await connection.query(
        `INSERT INTO finance_voucher_items (voucher_id, entry_type, subject_code, amount, summary, seq) 
         VALUES (?, '借', ?, ?, '收到学员学费', 0)`,
        [voucherId, subjectMapping['BANK_DEPOSIT'], amountNum]
      );
      // 贷：学员学费 - 确认收入
      await connection.query(
        `INSERT INTO finance_voucher_items (voucher_id, entry_type, subject_code, amount, summary, seq) 
         VALUES (?, '贷', ?, ?, '学员缴费收入', 1)`,
        [voucherId, subjectMapping['TUITION_INCOME'], amountNum]
      );
...
router.post('/refund', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 检查学员是否存在并获取当前金额信息
    const [studentRows] = await connection.query<RowDataPacket[]>(
      'SELECT id, contract_amount, actual_amount, discount_amount, debt_amount, account_balance, payment_status FROM students WHERE id = ?',
      [student_id]
    );
    ...
    await connection.query(
      'UPDATE students SET actual_amount = ?, debt_amount = ?, account_balance = ?, payment_status = ?, enrollment_status = ? WHERE id = ?',
      [newActualAmount, newDebtAmount, newAccountBalance, paymentStatus, '已退费', student_id]
    );

    // 记录退费信息（作为负数缴费记录）
    await connection.query(
      'INSERT INTO payment_records (student_id, amount, payment_date, payment_method, operator, notes) VALUES (?, ?, CURDATE(), ?, ?, ?)',
      [student_id, -refundAmount, '其他', operator, `退费：${notes || '学员退费'}`]
    );
...
router.post('/discount', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 检查学员是否存在并获取当前金额信息
    const [studentRows] = await connection.query<RowDataPacket[]>(
      'SELECT id, name, contract_amount, actual_amount, discount_amount, debt_amount, account_balance, payment_status FROM students WHERE id = ?',
      [student_id]
    );
    ...
    await connection.query(
      'UPDATE students SET discount_amount = ?, debt_amount = ?, account_balance = ?, payment_status = ?, enrollment_status = ? WHERE id = ?',
      [newDiscountAmount, newDebtAmount, newAccountBalance, paymentStatus, enrollmentStatus, student_id]
    );

    // 记录减免信息（作为负数缴费记录）
    await connection.query(
      'INSERT INTO payment_records (student_id, amount, payment_date, payment_method, operator, notes) VALUES (?, ?, CURDATE(), ?, ?, ?)',
      [student_id, -discountAmount, '其他', operator, `减免：${notes || '费用减免'}`]
    );

    // === 生成减免凭证 ===
    try {
      // 获取科目映射
      const subjectMapping = await getSubjectCodes(['TUITION_INCOME', 'OTHER_INCOME'], connection);
      ...
      const [voucherResult] = await connection.query<ResultSetHeader>(
        `INSERT INTO finance_vouchers (voucher_no, voucher_date, description, creator_id, creator_name, source_type, source_id) 
         VALUES (?, ?, ?, 0, ?, 'student_discount', ?)`,
        [voucherNo, today.toISOString().split('T')[0], `${student.name}费用减免`, operator, student_id]
      );
      const voucherId = voucherResult.insertId;
      ...
      await connection.query(
        `INSERT INTO finance_voucher_items (voucher_id, entry_type, subject_code, amount, summary, seq) 
         VALUES (?, '借', ?, ?, '学员费用减免', 0)`,
        [voucherId, subjectMapping['TUITION_INCOME'], discountAmount]
      );
      // 贷：其他收入 - 减免优惠
      await connection.query(
        `INSERT INTO finance_voucher_items (voucher_id, entry_type, subject_code, amount, summary, seq) 
         VALUES (?, '贷', ?, ?, '减免优惠', 1)`,
        [voucherId, subjectMapping['OTHER_INCOME'], discountAmount]
      );
...
router.delete('/:id', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 获取学员信息
    const [studentRows] = await connection.query<RowDataPacket[]>(
      'SELECT id, name, contract_amount, actual_amount, discount_amount, account_balance FROM students WHERE id = ?',
      [record.student_id]
    );
    ...
    // 同步更新缴费状态、报名状态、减免金额和账户余额
    await connection.query(
      'UPDATE students SET actual_amount = ?, discount_amount = ?, debt_amount = ?, account_balance = ?, payment_status = ?, enrollment_status = ? WHERE id = ?',
      [newActualAmount, newDiscountAmount, newDebtAmount, newAccountBalance, paymentStatus, enrollmentStatus, record.student_id]
    );
    ...
    if (recordAmount > 0) {
      try {
        ...
        const [voucherResult] = await connection.query<ResultSetHeader>(
          `INSERT INTO finance_vouchers (voucher_no, voucher_date, description, creator_id, creator_name, source_type, source_id) 
           VALUES (?, ?, ?, 0, '系统', 'payment_reversal', ?)`,
          [voucherNo, today.toISOString().split('T')[0], `${student.name}缴费冲销`, id]
        );
        const voucherId = voucherResult.insertId;
        ...
        await connection.query(
          `INSERT INTO finance_voucher_items (voucher_id, entry_type, subject_code, amount, summary, seq) 
           VALUES (?, '借', ?, ?, '冲销学员缴费收入', 0)`,
          [voucherId, subjectMapping['TUITION_INCOME'], recordAmount]
        );
        // 贷：银行存款 - 冲减资金流入
        await connection.query(
          `INSERT INTO finance_voucher_items (voucher_id, entry_type, subject_code, amount, summary, seq) 
           VALUES (?, '贷', ?, ?, '冲销收到学员学费', 1)`,
          [voucherId, subjectMapping['BANK_DEPOSIT'], recordAmount]
        );
    ...
    if (isDiscountRecord) {
      ...
      try {
        ...
        const [voucherResult] = await connection.query<ResultSetHeader>(
          `INSERT INTO finance_vouchers (voucher_no, voucher_date, description, creator_id, creator_name, source_type, source_id) 
           VALUES (?, ?, ?, 0, '系统', 'discount_reversal', ?)`,
          [voucherNo, today.toISOString().split('T')[0], `${student.name}减免冲销`, id]
        );
        const voucherId = voucherResult.insertId;
        ...
        await connection.query(
          `INSERT INTO finance_voucher_items (voucher_id, entry_type, subject_code, amount, summary, seq) 
           VALUES (?, '借', ?, ?, '冲销减免优惠', 0)`,
          [voucherId, subjectMapping['OTHER_INCOME'], discountReverseAmount]
        );
        // 贷：学员学费 - 恢复应收
        await connection.query(
          `INSERT INTO finance_voucher_items (voucher_id, entry_type, subject_code, amount, summary, seq) 
           VALUES (?, '贷', ?, ?, '恢复学员应收', 1)`,
          [voucherId, subjectMapping['TUITION_INCOME'], discountReverseAmount]
        );
...
// 获取缴费统计信息
router.get('/statistics/:studentId', async (req: Request, res: Response) => {
  try {
    const { studentId } = req.params;
    ...
    const [countRows] = await pool.query<RowDataPacket[]>(
      'SELECT COUNT(*) as payment_count FROM payment_records WHERE student_id = ?',
      [studentId]
    );

    // 获取最后一次缴费信息
    const [lastPaymentRows] = await pool.query<RowDataPacket[]>(
      'SELECT payment_date, amount FROM payment_records WHERE student_id = ? ORDER BY payment_date DESC, created_at DESC LIMIT 1',
      [studentId]
    );
...
router.post('/submit-confirm', async (req: Request, res: Response) => {
  const connection = await pool.getConnection();
  ...
  try {
    ...
    // 获取上缴配置（优先班型专属，回落全局）
    const config = await getActiveHeadquarterConfig(connection, student.class_type_id);
    ...
    // 计算上缴金额
    const submitAmount = calculateHeadquarterAmount(contractAmount, config);
    ...
    try {
      // 获取科目映射
      const subjectMapping = await getSubjectCodes(['BANK_DEPOSIT', 'HEADQUARTER_EXPENSE'], connection);
      ...
      const [voucherResult] = await connection.query<ResultSetHeader>(
        `INSERT INTO finance_vouchers (voucher_no, voucher_date, description, creator_id, creator_name, source_type, source_id) 
         VALUES (?, ?, ?, 0, ?, 'submit_confirm', ?)`,
        [voucherNo, submitDate, `${student.name}上缴总校款项`, operator, student_id]
      );
      const voucherId = voucherResult.insertId;
      ...
      await connection.query(
        `INSERT INTO finance_voucher_items (voucher_id, entry_type, subject_code, amount, summary, seq) 
         VALUES (?, '借', ?, ?, '上缴总校款项', 0)`,
        [voucherId, subjectMapping['HEADQUARTER_EXPENSE'], submitAmount]
      );
      // 贷：银行存款 - 资金流出
      await connection.query(
        `INSERT INTO finance_voucher_items (voucher_id, entry_type, subject_code, amount, summary, seq) 
         VALUES (?, '贷', ?, ?, '支付总校上缴款', 1)`,
        [voucherId, subjectMapping['BANK_DEPOSIT'], submitAmount]
      );
    ...
    await connection.query(
      `UPDATE students 
       ...
       WHERE id = ?`,
      [submitAmount, submitDate, operator, student_id]
    );

    // 记录上缴历史
    await connection.query(
      `INSERT INTO submit_records 
...
// 撤销上缴接口
router.post('/submit-revoke', async (req: Request, res: Response) => {
  const connection = await pool.getConnection();
  ...
  try {
    ...
    try {
      ...
      const [voucherResult] = await connection.query<ResultSetHeader>(
        `INSERT INTO finance_vouchers (voucher_no, voucher_date, description, creator_id, creator_name, source_type, source_id) 
         VALUES (?, ?, ?, 0, ?, 'submit_revoke', ?)`,
        [voucherNo, revokeDate, `${student.name}撤销上缴`, operator, student_id]
      );
      const voucherId = voucherResult.insertId;
...
// 获取上缴记录列表
router.get('/submit-records', async (req: Request, res: Response) => {
  try {
    const {
      limit = '10',
      offset = '0',
