  ...
  try {
    ...
    const [registrations] = await pool.query<RowDataPacket[]>(
      `SELECT 
       ...
       ${whereClause}
       ORDER BY es.exam_date DESC, er.registration_date DESC`,
      params
    );
...
router.post('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [schedules] = await connection.query<RowDataPacket[]>(
      'SELECT id, exam_type, capacity, arranged_count FROM exam_schedules WHERE id = ?',
      [exam_schedule_id]
    );
    ...
    const [progress] = await connection.query<RowDataPacket[]>(
      'SELECT exam_qualification FROM student_exam_progress WHERE student_id = ?',
      [student_id]
    );
    ...
    const [existing] = await connection.query<RowDataPacket[]>(
      'SELECT id FROM exam_registrations WHERE student_id = ? AND exam_schedule_id = ?',
    ...
    const [result] = await connection.query<ResultSetHeader>(
      `INSERT INTO exam_registrations (student_id, exam_schedule_id, notes)
       VALUES (?, ?, ?)`,
      [student_id, exam_schedule_id, notes]
    );

    // 更新考试安排的已安排人数
    await connection.query(
      'UPDATE exam_schedules SET arranged_count = arranged_count + 1 WHERE id = ?',
      [exam_schedule_id]
    );
...
router.put('/:id/result', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 更新考试结果
    await connection.query(
      'UPDATE exam_registrations SET exam_result = ?, exam_score = ?, notes = ? WHERE id = ?',
      [exam_result, exam_score, notes, id]
    );

    // 确保学员有进度记录
    const [progressCheck] = await connection.query<RowDataPacket[]>(
      'SELECT id FROM student_exam_progress WHERE student_id = ?',
      [student_id]
    );

    if (progressCheck.length === 0) {
      await connection.query(
        'INSERT INTO student_exam_progress (student_id) VALUES (?)',
        [student_id]
      );
    }
    ...
    if (exam_result === '通过') {
      ...
      const [updated] = await connection.query<RowDataPacket[]>(
        `SELECT ${subjectPrefix}_failed_count as failed_count FROM student_exam_progress WHERE student_id = ?`,
        [student_id]
      );
      ...
      if (failedCount === 3) {
        // 3次预警
        await connection.query(
          `INSERT INTO exam_warning_logs (student_id, warning_type, warning_subject, warning_content)
           VALUES (?, '3次预警', ?, ?)`,
          [student_id, exam_type, `学员${exam_type}已连续3次未通过，请注意考试安排`]
        );
        ...
        await connection.query(
          `INSERT INTO exam_warning_logs (student_id, warning_type, warning_subject, warning_content)
           VALUES (?, '4次预警', ?, ?)`,
          [student_id, exam_type, `⚠️ 学员${exam_type}已连续4次未通过，请慎重约考第5次考试`]
        );
        ...
        await connection.query(
          `UPDATE student_exam_progress 
           SET exam_qualification = '已作废',
               ...
               disqualified_reason = ?
           WHERE student_id = ?`,
          [`${exam_type}连续5次未通过`, student_id]
        );

        // 更新学员主表状态为"废考"
        await connection.query(
          `UPDATE students SET enrollment_status = '废考' WHERE id = ?`,
          [student_id]
        );
        ...
        await connection.query(
          `INSERT INTO exam_warning_logs (student_id, warning_type, warning_subject, warning_content)
           VALUES (?, '资格作废', ?, ?)`,
          [student_id, exam_type, `❌ 学员${exam_type}连续5次未通过，驾考资格已作废`]
        );
      }
    }

    // 重新计算总进度
    const [finalProgress] = await connection.query<RowDataPacket[]>(
      `SELECT 
         subject1_status, subject2_status, subject3_status, subject4_status
       FROM student_exam_progress WHERE student_id = ?`,
      [student_id]
    );

    let totalProgress = 0;
    if (finalProgress[0].subject1_status === '已通过') totalProgress += 25;
    if (finalProgress[0].subject2_status === '已通过') totalProgress += 25;
    if (finalProgress[0].subject3_status === '已通过') totalProgress += 25;
    if (finalProgress[0].subject4_status === '已通过') totalProgress += 25;

    await connection.query(
      'UPDATE student_exam_progress SET total_progress = ? WHERE student_id = ?',
      [totalProgress, student_id]
    );
...
router.delete('/:id', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 获取报名记录
    const [registrations] = await connection.query<RowDataPacket[]>(
      'SELECT exam_schedule_id FROM exam_registrations WHERE id = ?',
      [id]
    );
