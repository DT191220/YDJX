...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const sortColumn = validSortColumns.includes(sortBy as string) ? sortBy : 'salary_month';
...
router.post('/generate', async (req: Request, res: Response) => {
  try {
    ...
    const [coaches] = await pool.query<RowDataPacket[]>(
      "SELECT id, name FROM coaches WHERE status = '在职'"
    );
    ...
    for (const coach of coaches) {
      ...
      const [existing] = await pool.query<RowDataPacket[]>(
        'SELECT id FROM coach_monthly_salary WHERE coach_id = ? AND salary_month = ?',
        [coach.id, salary_month]
      );
...
router.post('/refresh', async (req: Request, res: Response) => {
  try {
    ...
    // 获取该月份的所有工资记录
    const [salaries] = await pool.query<RowDataPacket[]>(
      'SELECT * FROM coach_monthly_salary WHERE salary_month = ?',
      [salary_month]
    );
...
router.delete('/batch', async (req: Request, res: Response) => {
  try {
    ...
    if (!salary_month || !/^\d{4}-\d{2}$/.test(salary_month as string)) {
      return res.status(400).json({
        success: false,
        message: '请提供有效的月份（格式：YYYY-MM）'
      });
    ...
    // 检查是否有已发放的工资记录
    const [paidRecords] = await pool.query<RowDataPacket[]>(
      "SELECT COUNT(*) as count FROM coach_monthly_salary WHERE salary_month = ? AND status = 'paid'",
      [salary_month]
    );
    ...
    const [result] = await pool.query<ResultSetHeader>(
      'DELETE FROM coach_monthly_salary WHERE salary_month = ?',
      [salary_month]
    );
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const [record] = await pool.query<RowDataPacket[]>(
      'SELECT status FROM coach_monthly_salary WHERE id = ?',
      [id]
    );
    ...
    await pool.query('DELETE FROM coach_monthly_salary WHERE id = ?', [id]);
