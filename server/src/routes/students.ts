...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const {
      name, id_card, phone, gender, address, native_place,
      status, enrollment_date, medical_history,
      has_driving_experience, driving_years, previous_school_history,
      emergency_contact, emergency_phone, coach_id, coach_name,
      coach_subject2_name, coach_subject3_name, class_type_id, license_type
    } = req.body;
    ...
    const [existing] = await pool.query(
      'SELECT id FROM students WHERE id_card = ?',
      [id_card]
    );
    ...
    const registrar_id = (req as any).user?.id || null;
    const registrar_name = (req as any).user?.username || null;
    ...
    if (class_type_id) {
      const [classTypes] = await pool.query(
        'SELECT contract_amount FROM class_types WHERE id = ?',
        [class_type_id]
      );
      if ((classTypes as any[]).length > 0) {
        // 快照当前班型价格作为学员的合同金额（历史价格）
        contractAmount = (classTypes as any[])[0].contract_amount;
      }
    }
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const [duplicate] = await pool.query(
      'SELECT id FROM students WHERE id_card = ? AND id != ?',
      [id_card, id]
    );
    ...
    const [currentStudentRows] = await pool.query(
      'SELECT enrollment_status, payment_status, contract_amount, enrollment_date, license_type FROM students WHERE id = ?',
      [id]
    );
    
    const currentStudent = (currentStudentRows as any[])[0];
...
// 获取招生统计数据
router.get('/statistics/enrollment', async (req: Request, res: Response) => {
  try {
    const { year, month } = req.query;
    const targetYear = year ? parseInt(year as string) : new Date().getFullYear();
    const targetMonth = month ? parseInt(month as string) : 0; // 0表示全年

    // 构建日期筛选条件
    let dateCondition = 'YEAR(enrollment_date) = ?';
    let dateParams: any[] = [targetYear];
    ...
    // 按班型统计
    const [byClassType] = await pool.query(`
      SELECT 
    ...
    // 按教练统计
    const [byCoach] = await pool.query(`
      SELECT 
        coach_name,
        COUNT(*) as count
      FROM students
    ...
    // 教练-班型交叉统计（新增）
    const [coachClassType] = await pool.query(`
      SELECT 
        s.coach_name,
        ct.name as class_type_name,
        COUNT(*) as count
    ...
    if (targetMonth === 0) {
      ...
      for (let m = 1; m <= 12; m++) {
        ...
        const found = (monthlyData as any[]).find(item => item.month === monthStr);
        monthlyStats.push({
          month: monthStr,
          monthName: `${m}月`,
    ...
    if (targetMonth > 0) {
      ...
      lastPeriodTotal = (lastMonthTotal as any[])[0]?.total || 0;
    } else {
      // 去年全年
      const [lastYearTotal] = await pool.query(`
        SELECT COUNT(*) as total
        FROM students
        WHERE YEAR(enrollment_date) = ?
          AND enrollment_status NOT IN ('咨询中', '预约报名')
      `, [targetYear - 1]);
      lastPeriodTotal = (lastYearTotal as any[])[0]?.total || 0;
    }
    ...
    res.json({
      ...
      data: {
        year: targetYear,
        month: targetMonth,
        currentTotal: (currentTotal as any[])[0]?.total || 0,
        yearTotal: (yearTotal as any[])[0]?.total || 0,
        lastPeriodTotal,
        monthlyStats,
        byClassType,
        byCoach,
