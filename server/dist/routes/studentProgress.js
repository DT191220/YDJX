...
router.get('/', async (req, res) => {
    try {
        const { keyword = '', subject1_status = '', subject2_status = '', subject3_status = '', subject4_status = '' } = req.query;
        ...
        const [progress] = await database_1.default.query(`SELECT 
         sep.*,
       ...
       ${whereClause}
       ORDER BY sep.updated_at DESC`, params);
        res.json({
            success: true,
    ...
    try {
       ...
       WHERE sep.student_id = ?`, [studentId]);
        if (progress.length === 0) {
            // 如果学员没有进度记录，自动创建一条
            const [student] = await database_1.default.query('SELECT id, name, license_type FROM students WHERE id = ?', [studentId]);
            if (student.length === 0) {
            ...
            const [result] = await database_1.default.query(`INSERT INTO student_exam_progress (student_id, license_type) VALUES (?, ?)`, [studentId, student[0].license_type]);
            const [newProgress] = await database_1.default.query(`SELECT 
           sep.*,
           s.name as student_name,
           s.phone as student_phone,
           s.license_type
...
router.put('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { subject1_status, subject1_pass_date, subject2_status, subject2_pass_date, subject3_status, subject3_pass_date, subject4_status, subject4_pass_date } = req.body;
        // 先获取当前状态，用于判断状态变化
        const [currentProgress] = await database_1.default.query(`SELECT * FROM student_exam_progress WHERE id = ?`, [id]);
        ...
        // 计算总进度百分比
        let totalProgress = 0;
        if (subject1_status === '已通过')
            totalProgress += 25;
        if (subject2_status === '已通过')
            totalProgress += 25;
        if (subject3_status === '已通过')
            totalProgress += 25;
        if (subject4_status === '已通过')
            totalProgress += 25;
        // 计算考试次数和连挂次数的变化
        // 辅助函数：计算单科目的统计变化
        const calcSubjectStats = (oldStatus, newStatus, oldTotalCount, oldFailedCount) => {
        ...
        const subject1Stats = calcSubjectStats(current.subject1_status, subject1_status, current.subject1_total_count || 0, current.subject1_failed_count || 0);
        const subject2Stats = calcSubjectStats(current.subject2_status, subject2_status, current.subject2_total_count || 0, current.subject2_failed_count || 0);
        const subject3Stats = calcSubjectStats(current.subject3_status, subject3_status, current.subject3_total_count || 0, current.subject3_failed_count || 0);
        const subject4Stats = calcSubjectStats(current.subject4_status, subject4_status, current.subject4_total_count || 0, current.subject4_failed_count || 0);
        await database_1.default.query(`UPDATE student_exam_progress 
       ...
       WHERE id = ?`, [
        ...
        // 返回更新后的完整数据
        const [updatedProgress] = await database_1.default.query(`SELECT 
         sep.*,
         s.name as student_name,
         s.phone as student_phone,
         s.id_card as student_id_card,
...
// 检查学员报考资格
router.get('/check-eligibility/:studentId/:subject', async (req, res) => {
    try {
        const { studentId, subject } = req.params;
        // 检查学员是否存在
        const [students] = await database_1.default.query('SELECT id, enrollment_status FROM students WHERE id = ?', [studentId]);
        ...
        // 检查考试进度
        const [progress] = await database_1.default.query('SELECT * FROM student_exam_progress WHERE student_id = ?', [studentId]);
        if (progress.length === 0) {
            // 没有进度记录,可以报考
            return res.json({
                success: true,
...
router.get('/:id/warnings', async (req, res) => {
    try {
        ...
        const [progress] = await database_1.default.query(`SELECT 
         ...
         exam_qualification
       FROM student_exam_progress WHERE id = ?`, [id]);
        if (progress.length === 0) {
            return res.json({
                success: true,
                data: {
