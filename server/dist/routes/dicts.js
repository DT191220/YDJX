...
router.get('/types', async (req, res) => {
    try {
        ...
        // 获取总数
        const [countResult] = await database_1.default.query(`SELECT COUNT(*) as total FROM sys_dict_types ${whereClause}`, params);
        ...
        const [types] = await database_1.default.query(`SELECT id, dict_name, dict_type, status, description, created_at, updated_at
       FROM sys_dict_types 
       ${whereClause} 
       ${orderClause} 
...
router.get('/types/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const [types] = await database_1.default.query('SELECT * FROM sys_dict_types WHERE id = ?', [id]);
...
router.post('/types', async (req, res) => {
    try {
        ...
        // 检查字典类型是否已存在
        const [existing] = await database_1.default.query('SELECT id FROM sys_dict_types WHERE dict_type = ?', [dict_type]);
        ...
        const [result] = await database_1.default.query(`INSERT INTO sys_dict_types (dict_name, dict_type, status, description) 
       VALUES (?, ?, ?, ?)`, [dict_name, dict_type, status || '启用', desc || null]);
        res.status(201).json({
            success: true,
            message: '创建字典类型成功',
...
router.put('/types/:id', async (req, res) => {
    try {
        ...
        // 检查字典类型是否存在
        const [existing] = await database_1.default.query('SELECT id FROM sys_dict_types WHERE id = ?', [id]);
        if (existing.length === 0) {
            return res.status(404).json({
                success: false,
                message: '字典类型不存在'
        ...
        // 检查字典类型是否与其他记录冲突
        const [duplicate] = await database_1.default.query('SELECT id FROM sys_dict_types WHERE dict_type = ? AND id != ?', [dict_type, id]);
        ...
        await database_1.default.query(`UPDATE sys_dict_types 
       SET dict_name = ?, dict_type = ?, status = ?, description = ?
       WHERE id = ?`, [dict_name, dict_type, status, desc || null, id]);
        res.json({
            success: true,
            message: '更新字典类型成功'
        });
...
router.delete('/types/:id', async (req, res) => {
    try {
        ...
        const [typeResult] = await database_1.default.query('SELECT dict_type FROM sys_dict_types WHERE id = ?', [id]);
        ...
        await database_1.default.query('DELETE FROM sys_dict_types WHERE id = ?', [id]);
        res.json({
            success: true,
            message: '删除字典类型成功'
        });
...
router.get('/by-type/:dictType', async (req, res) => {
    try {
       ...
       ORDER BY sort_order ASC, id ASC`, [dictType]);
        res.json({
            success: true,
            message: '获取字典数据成功',
            data: data
...
router.post('/data', async (req, res) => {
    try {
        ...
        const [typeExists] = await database_1.default.query('SELECT dict_type FROM sys_dict_types WHERE id = ?', [dict_type_id]);
